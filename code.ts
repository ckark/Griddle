...
figma.on('run', ({ parameters }: RunEvent) => {
	...
	const origX = figma.currentPage.selection.map((e) => e.x).reduce((a, b) => Math.min(a, b)),
		...
		grid = (e, a) => {
			...
			for (let e = 0; e < items.length; e++) for (let t = 0; t < itemXPos.length; t++) (items[e].x = itemXPos[t] + origX), e++;
			for (let e = 0; e < items.length; e++) for (let t = 0; t < itemYPos.length; t++) (items[e].y = itemYPos[t] + origY), e++;
			l.remove();
		},
		singleRow = (e, t) => {
