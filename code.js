...
figma.on('run', ({ parameters }) => {
    ...
    const sorted = () => {
        ...
        if ('Descending' === parameters.sort)
            o.sort((e, o) => e.node.name.toLocaleLowerCase().localeCompare(o.node.name.toLocaleLowerCase(), void 0, { numeric: !0 }));
        else {
            if ('Ascending' !== parameters.sort)
                return;
        ...
        let r = [], t = [...figma.currentPage.selection].sort((e, r) => e.x - r.x).sort((e, r) => e.y - r.y);
        if ('No' === parameters.sort)
            for (e = t; e.length;)
                r.push(e.splice(0, a));
        else if ('Descending' === parameters.sort)
            for (e = t.sort((e, r) => e.name.localeCompare(r.name)); e.length;)
                r.push(e.splice(0, a));
        else if ('Ascending' === parameters.sort)
            for (e = t.sort((e, r) => r.name.localeCompare(e.name)); e.length;)
                r.push(e.splice(0, a));
        let o = [];
        ...
        for (let e of r) {
            let r = l();
            for (let o of e)
                r.appendChild(o), t.push(o.parent);
            r.layoutMode = 'NONE';
        }
        ...
        for (let e of f)
            n.push(e.parent), s.parent.appendChild(e);
        const p = [], i = [];
        for (let e of n)
            i.push(e.y);
        for (let e of f)
