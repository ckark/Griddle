...
figma.on('run', ({ parameters }) => {
    const sorted = () => {
    ...
    }, origX = figma.currentPage.selection.map((e) => e.x).reduce((e, a) => Math.min(e, a)), origY = figma.currentPage.selection.map((e) => e.y).reduce((e, a) => Math.min(e, a)), format = (e) => {
        ...
        let r = [], n = [...figma.currentPage.selection].sort((e, a) => e.x - a.x).sort((e, a) => e.y - a.y);
        if ('No' === parameters.sort)
            for (e = n; e.length;)
                r.push(e.splice(0, a));
        else if ('Descending' === parameters.sort)
            for (e = n.sort((e, a) => e.name.localeCompare(a.name)); e.length;)
                r.push(e.splice(0, a));
        else if ('Ascending' === parameters.sort)
            for (e = n.sort((e, a) => a.name.localeCompare(e.name)); e.length;)
                r.push(e.splice(0, a));
